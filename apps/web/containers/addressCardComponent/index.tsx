...
import { GET_ADDRESS, GET_ORDER } from "@/app/queries";
    ...
    <>
      {getOrderResponse?.data && (
        <div className="flex flex-col justify-between gap-3 px-5 py-5">
          <AddressCard
            address={getOrderResponse?.data?.getOrder?.invoice?.shippingAddress}
            type="Shipping Address"
          />
          <AddressCard
            address={getOrderResponse?.data?.getOrder?.invoice?.billingAddress}
...
const AddressCard = ({ address, type }: { address: any; type: string }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6 border-t-4 border-[black] w-[100%]">
      <h2 className="text-xl font-bold text-gray-800 mb-2 flex lfex-row justify-center w-[100%]">
        {type}
      </h2>
      ...
      <p className="text-gray-600 mb-1">
        <span className="font-semibold">Address:</span> {address?.addressLine1},{" "}
...
const AddressContainer = () => {
  const { recordId } = useParams();
  const [getAddress, getAddressResponse] = useLazyQuery(serverFetch);
  useEffect(() => {
    getAddress(
      GET_ADDRESS,
      {
  ...
  return (
    <div className="px-5 py-5">
      <AddressCard
        address={getAddressResponse?.data?.getAddress}
        type="Address"
      />
    </div>
  );
