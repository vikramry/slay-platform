...
export function DeletePopupComp({
  inputText,
  type,
  onclick,
  ...rest
  ...
  return (
    <AlertDialogContent>
      <AlertDialogHeader>
        <AlertDialogTitle>
          Are you sure you want to drop {typeMap[type]} "{inputText}"?
        </AlertDialogTitle>
        <AlertDialogDescription>
          <InputField
            type="text"
            label={`Type "${inputText}" to confirm your action`}
            placeholder={`Enter ${typeMap[type]} name for delete`}
            onChange={(e) => setConfirmDeleteText(e.target.value)}
            classNames="border-[1.5px]  dark:border-[#18181b]/80  dark:text-[red]"
          />
        </AlertDialogDescription>
      </AlertDialogHeader>
      <AlertDialogFooter>
        <AlertDialogCancel>Cancel</AlertDialogCancel>
        <AlertDialogAction className="bg-transparent shadow-none border-none hover:bg-transparent">
          <CustomButton
            buttonText="Delete"
            variant="primary"
            size="md"
